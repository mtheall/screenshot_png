...
static void
png_file_write(png_structp png_ptr,
               png_bytep   data,
               png_size_t  length)
{
  ssize_t rc;
...
static void
png_file_error(png_structp     png_ptr,
...
static void
png_file_warning(png_structp     png_ptr,
...
#ifdef USE_CALLBACK
static void
png_row_callback(png_structp png_ptr,
                 png_uint_32 row,
                 int         pass)
{
  fputc('.', stderr);
...
static inline u8*
get_pixel(u8 *fb, u16 x, u16 y, u16 w, u16 h, u32 bpp)
{
  return &fb[(x*w + (w-y-1))*bpp];
}

static void
fill_png_buffer(void)
{
  size_t x, y;
  u8     *p = png_buffer;
...
int
screenshot_png(const char *path,
               int        level)
{
  size_t      i;
  png_structp png_ptr;
  ...
  png_set_IHDR(png_ptr, info_ptr, 400, 480, 8, PNG_COLOR_TYPE_RGB_ALPHA,
